...
test("picking the two calipers asks exactly the caliper questions", async ({ page }) => {
  await page.goto(
    `${href("fr", "/velo/[id]/controle", { id: "demo" })}?parts=brake-caliper-front,brake-caliper-rear`,
  );

  await expect(page.getByTestId("checkup-scope")).toHaveText("Contrôle partiel");
...
test("a KO with a symptom becomes one line of the list, across a reload", async ({ page }) => {
  await page.goto(`${href("fr", "/velo/[id]/controle", { id: "demo" })}?parts=chain`);
  await page.getByTestId("checkup-start").click();
...
test("the resume banner appears on the bike, and goes once the checkup is done", async ({
  page,
}) => {
  await page.goto(`${href("fr", "/velo/[id]/controle", { id: "demo" })}?parts=chain`);
  await page.getByTestId("checkup-start").click();
  ...
  await expect(page.getByTestId("resume-cta")).toHaveAttribute(
    "href",
    `${href("fr", "/velo/[id]/controle", { id: "demo" })}?parts=chain`,
  );
  await page.goto(`${href("fr", "/velo/[id]/controle", { id: "demo" })}?parts=chain`);
  await expect(page.getByTestId("checkup-summary")).toBeVisible();
  await page.getByTestId("summary-create").click();
...
test("a scope that matches no question says so instead of asking nothing", async ({ page }) => {
  ...
  await page.goto(`${href("fr", "/velo/[id]/controle", { id: "demo" })}?parts=rack`);
