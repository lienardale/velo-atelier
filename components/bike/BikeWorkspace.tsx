 ...
 * two rows per part, for about half a second (`.debug/005`). Selection is set
...
function useLocalBike(enabled: boolean): LocalBikeState {
  ...
  // §6.7: `/velo/local` without storage goes home. `replace`, not `push`, so
  // Back does not bounce the visitor straight into the empty page again.
  useEffect(() => {
    if (missing) router.replace({ pathname: "/" });
  }, [missing, router]);
